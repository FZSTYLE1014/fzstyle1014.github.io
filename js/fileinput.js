  ...
  $h = {
    ...
    cspBuffer: {
      ...
      stash: function (htmlString) {
        ...
        $.each(self.domEventsList, function (key, eventName) {
          // handle onXXX events set as inline markup
          var id,
            fn,
            event = "on" + eventName,
            $inlineEvent = $el.find("[" + event + "]");
          if ($inlineEvent.length) {
            ...
            self.domElementEvents[id].push({
              name: eventName + ".csp",
              handler: fn,
            }); // special csp namespace
  ...
  FileInput.prototype = {
      ...
      self.fileManager = {
        ...
        getUploadStats: function (id, loaded, total) {
          var fm = self.fileManager,
            started = id
              ? (fm.stats[id] && fm.stats[id].started) || null
              : null;
          if (!started) {
    ...
    _initResumableUpload: function () {
      ...
      self.resumableManager = {
        ...
        getProgress: function () {
          var rm = self.resumableManager,
            chunksProcessed = rm.processedResumables(),
            total = rm.chunkCount;
          if (total === 0) {
            return 0;
          }
        ...
        uploadResumable: function () {
          ...
          pool
            .run(self.resumableUploadOptions.maxThreads)
            .done(function () {
