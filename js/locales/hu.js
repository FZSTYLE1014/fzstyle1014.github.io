...
(function ($) {
  ...
  $.fn.fileinputLocales["hu"] = {
    ...
    msgSizeTooSmall:
      'A fájl: "{name}" (<b>{size} KB</b>) mérete túl kicsi, nagyobbnak kell lennie, mint <b>{minSize} KB</b>.',
    msgSizeTooLarge:
      '"{name}" fájl (<b>{size} KB</b>) mérete nagyobb a megengedettnél <b>{maxSize} KB</b>.',
    msgFilesTooLess:
      "Legalább <b>{n}</b> {files} ki kell választania a feltöltéshez.",
    msgFilesTooMany:
      "A feltölteni kívánt fájlok száma <b>({n})</b> elérte a megengedett maximumot <b>{m}</b>.",
    msgTotalFilesTooMany:
      "Legfeljebb <b>{m}</b> fájlt tölthet fel (<b>{n}</b> fájlt észlel).",
    msgFileNotFound: '"{name}" fájl nem található!',
    msgFileSecured:
      'Biztonsági beállítások nem engedik olvasni a fájlt "{name}".',
    ...
    msgInvalidFileName:
      'Hibás vagy nem támogatott karakterek a fájl nevében "{name}".',
    msgInvalidFileType:
      'Nem megengedett fájl "{name}". Csak a "{types}" fájl típusok támogatottak.',
    msgInvalidFileExtension:
      'Nem megengedett kiterjesztés / fájltípus "{name}". Csak a "{extensions}" kiterjesztés(ek) / fájltípus(ok) támogatottak.',
    msgFileTypes: {
      image: "image",
      html: "HTML",
      text: "text",
    ...
    msgImageWidthSmall:
      'A kép szélességének "{name}" legalább {size} pixelnek kell lennie.',
    msgImageHeightSmall:
      'A kép magasságának "{name}" legalább {size} pixelnek kell lennie.',
    msgImageWidthLarge:
      'A kép szélessége "{name}" nem haladhatja meg a {size} pixelt.',
    msgImageHeightLarge:
      'A kép magassága "{name}" nem haladhatja meg a {size} pixelt.',
    msgImageResizeError:
      "Nem lehet megállapítani a kép méreteit az átméretezéshez.",
    msgImageResizeException:
      "Hiba történt a méretezés közben.<pre>{errors}</pre>",
    msgAjaxError:
      "Hiba történt a művelet közben ({operation}). Kérjük, próbálja később!",
    msgAjaxProgressError: "Hiba! ({operation})",
    msgDuplicateFile:
      'A (z) {name} "azonos méretű" {size} KB "fájlt már korábban kiválasztották. Az ismétlődő kiválasztás kihagyása.',
    msgResumableUploadRetriesExceeded:
      "Feltöltés megszakítva a <b> {fájl} </b> fájl <b> {max} </b> próbálkozásain túl! Hiba részletei: <pre>{error}</pre>",
    ...
    dropZoneClickTitle:
      "<br>(vagy kattintson ide a {files} tallózásához &hellip;)",
    fileActionSettings: {
